...
"""Fine-tuning the library models for causal language modeling (GPT, GPT-2,
CTRL, ...) on a text file or a dataset.
...
def main():
    ...
    if data_args.dataset_name is not None:
        ...
        extension = (
            data_args.train_file.split('.')[-1] if data_args.train_file
            is not None else data_args.validation_file.split('.')[-1])
        if extension == 'txt':
            extension = 'text'
    ...
    from mmrazor.implementations.pruning.dms.core.models.opt.opt_wrapper import (
        DmsOptAlgorithm, to_static_model)
    ...
    model: OPTForCausalLM = to_static_model(model)
    model.tie_weights()
