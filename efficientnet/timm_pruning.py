...
from timm.data import (AugMixDataset, FastCollateMixup, Mixup, create_loader,
                       resolve_data_config)
...
parser.add_argument(
    '--data-dir', metavar='DIR', help='path to dataset (root dir)')
parser.add_argument(
    '--dataset',
    metavar='NAME',
    default='',
...
group.add_argument(
    '--model-kwargs', nargs='*', default={}, action=utils.ParseKwargs)
group.add_argument(
    '--head-init-scale',
    default=None,
    type=float,
...
group.add_argument(
    '--opt-kwargs', nargs='*', default={}, action=utils.ParseKwargs)
...
def main():
    ...
    try:
        for epoch in range(start_epoch, num_epochs):
            ...
            elif args.distributed and hasattr(loader_train.sampler,
                                              'set_epoch'):
    ...
    for batch_idx, (input, target) in enumerate(loader):
        ...
        if update_idx % args.log_interval == 0:
            ...
            if args.distributed:
                ...
                losses_m.update(reduced_loss.item() * accum_steps,
                                input.size(0))
...
def validate(model,
    ...
    with torch.no_grad():
        for batch_idx, (input, target) in enumerate(loader):
            ...
            if utils.is_primary(args) and (last_batch or
